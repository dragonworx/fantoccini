...
export class DynamicStyleSheet {
  ...
  private parse(node: CSSRuleNode, selectorPrefix?: string) {
    ...
    const cheekyNode = node as any;
    let selector = cheekyNode.selector as string;
    const isSelfReferencing = selector[0] === '&';
    if (isSelfReferencing) {
      selector = selector.substring(1);
    }
    const rules = cheekyNode.rules as { [Property in CSSRuleKey]?: string };
    const children = cheekyNode.children as CSSRuleNode[];
